...
export default function DashboardPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data: { id: string; prompt: string; message: string; actions: BuildAction[]; status: "planned" | "chat" } = await res.json();

      setMessages((prev) => [...prev, { id: `a_${data.id}`, role: "assistant", content: data.message, actions: data.actions }]);
    ...
    <div className="mx-auto flex h-full max-w-3xl flex-col px-6 py-8">
                  ...
                  <motion.div key={msg.id} initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.3, ease: "easeOut" }} className="flex justify-start">
                    <div className={`max-w-[80%] rounded-2xl rounded-tl-sm border px-4 py-3 text-sm ${msg.isError ? "border-red-200 bg-red-50 text-red-700" : "border-border bg-surface text-foreground"}`}>
                      ...
                      {msg.actions && msg.actions.length > 0 && (
                        <motion.div className="mt-2.5 flex flex-col gap-1.5 border-t border-border pt-2.5" initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }}>
      ...
      <motion.div layout className="mt-4 rounded-xl border border-border bg-surface transition-all focus-within:border-neutral-400 focus-within:ring-4 focus-within:ring-neutral-100">
        ...
        <div className="flex items-center justify-between border-t border-border px-4 py-2.5">
          ...
          <motion.button whileTap={{ scale: 0.96 }} onClick={handleGenerate} disabled={!prompt.trim() || isGenerating} className="flex items-center gap-1.5 rounded-lg bg-accent px-3.5 py-1.5 text-sm font-medium text-white transition-colors hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-40">
